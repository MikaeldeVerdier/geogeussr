...
class Trainer:
    ...
    def train(self, model, iteration_amount):
        for _ in range(iteration_amount):
            batch = self.dataset_handler.generate_batch(model.preprocess_func, model.input_shape, completion=lambda x: self.conditional_add_regressor(model, x))  # wish we had swift closure instead...
            x, y = zip(*batch)
